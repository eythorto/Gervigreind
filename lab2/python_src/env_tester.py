...
def bfs_traverse(env, expected_nb_states):
  ...
  while len(open_list)>0:
    if n_visited > expected_nb_states:
      ...
      break
    if n_visited>0 and n_visited % 10000 == 0:
      print("generated %d states (%.0f%% of %d expected) and going ..." % (n_visited, 100.0*n_visited/expected_nb_states, expected_nb_states))
    state = open_list.pop()
    if not env.is_goal_state(state):
      actions = env.get_legal_actions(state)
      n += 1
      branching_factor += 1.0 / n * (len(actions) - branching_factor)
      for next_state in [env.get_next_state(state, a) for a in actions]:
        if next_state not in visited:
          visited[next_state] = 1
          n_visited += 1
          open_list.append(next_state)
  return list(visited.keys()), branching_factor
...
def main(argv):
  ...
  env = Environment(width,height,nb_dirts)
  states, branching_factor = bfs_traverse(env, expected_nb_states)
  end_time = time.process_time_ns()
  if expected_nb_states < len(states): # generated more than expected
    return
  ...
  print("The average branching factor is %.1f (legal actions per state)." % branching_factor)
  ...
  print()
  states2, _ = bfs_traverse(env, expected_nb_states)
  nb_errors = 0
